...
| Remove a screen | *(not yet built — see below)* | Structurally understood (Screen State Model), not yet wired into `fit_patch.py` |
...
user to the on-device menu. Two caveats carried forward: any NewFiles
...
Model), and `--un-remove`'s own live round-trip is not yet re-verified
under the new default (only `--new-slot` has been).
