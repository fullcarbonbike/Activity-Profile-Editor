...
- **Connect IQ data fields (third-party apps like WindField) can't be
  ...
  already has one. **Favorite Screen is covered by this too:** saving a
  favorite from a screen holding a Connect IQ field, then loading it
  into a new screen, does *not* carry that field over — confirmed on
  real hardware, it comes up as "Timer" like every other route. Saving
  such a favorite is now refused outright, and any favorite saved by an
  earlier version has its Connect IQ entries dropped (with a notice) at
  load time. See `PROJECT_NOTES.md` Doc rev 95-98 and 101-102 for the
  full investigation.
